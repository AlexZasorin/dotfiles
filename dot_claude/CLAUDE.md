...
- 2026-03-26: Debugged broken CI E2E tests in platform-agent after package bumps. Root causes: (1) pnpm lockfile override (`ai@<5.0.52: '>=5.0.52'`) forced ai SDK from v4→v6, (2) react/react-dom floated from 19.1→19.2 due to caret ranges, (3) `@types/node` divergence (22.x vs 25.x) caused pnpm to create two copies of core-react@9.56.1 with separate SecurityContexts. Fixes: removed ai override, pinned react/react-dom to exact 19.1.0, added Vite `resolve.dedupe` for react, react-dom, and @risk-and-safety/core-react. User preferred deduplication fix over downgrading core-react.
